...
func BenchmarkToTime(b *testing.B) {
	...
	// string/cast-8       3559998 324 ns/op
	b.Run("string", func(b *testing.B) {
		...
		b.Run("cast", func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				_, err := cast.ToTimeE(v, cast.TimeFormat(format))
				if err != nil {
					b.Fatal(err)
				}
			}
...
func TestToTime(t *testing.T) {
	...
	assert.Equal(t, cast.ToTime(uint(3), cast.TimeFormat("ns")), time.Unix(0, 3))
	assert.Equal(t, cast.ToTime(uint8(3), cast.TimeFormat("ns")), time.Unix(0, 3))
	assert.Equal(t, cast.ToTime(uint16(3), cast.TimeFormat("ns")), time.Unix(0, 3))
	assert.Equal(t, cast.ToTime(uint32(3), cast.TimeFormat("ns")), time.Unix(0, 3))
	assert.Equal(t, cast.ToTime(uint64(3), cast.TimeFormat("ns")), time.Unix(0, 3))
	assert.Equal(t, cast.ToTime(cast.UintPtr(3), cast.TimeFormat("s")), time.Unix(3, 0))
	assert.Equal(t, cast.ToTime(cast.Uint8Ptr(3), cast.TimeFormat("s")), time.Unix(3, 0))
	assert.Equal(t, cast.ToTime(cast.Uint16Ptr(3), cast.TimeFormat("s")), time.Unix(3, 0))
	assert.Equal(t, cast.ToTime(cast.Uint32Ptr(3), cast.TimeFormat("s")), time.Unix(3, 0))
	assert.Equal(t, cast.ToTime(cast.Uint64Ptr(3), cast.TimeFormat("s")), time.Unix(3, 0))

	assert.Equal(t, cast.ToTime(float32(3), cast.TimeFormat("ns")), time.Unix(0, 3))
	assert.Equal(t, cast.ToTime(float64(3), cast.TimeFormat("ns")), time.Unix(0, 3))
	...
	{
		got := cast.ToTime("2022-09-30 15:30:00", cast.TimeFormat("2006-01-02 15:04:05"))
