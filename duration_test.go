...
func BenchmarkToDuration(b *testing.B) {
	...
	// string/cast-8       18037459 66.7 ns/op
...
func TestToDuration(t *testing.T) {
	...
	assert.Equal(t, cast.ToDuration(uint(3), cast.TimeFormat("ms")), 3*time.Millisecond)
	assert.Equal(t, cast.ToDuration(uint8(3), cast.TimeFormat("ms")), 3*time.Millisecond)
	assert.Equal(t, cast.ToDuration(uint16(3), cast.TimeFormat("ms")), 3*time.Millisecond)
	assert.Equal(t, cast.ToDuration(uint32(3), cast.TimeFormat("ms")), 3*time.Millisecond)
	assert.Equal(t, cast.ToDuration(uint64(3), cast.TimeFormat("ms")), 3*time.Millisecond)
	assert.Equal(t, cast.ToDuration(cast.UintPtr(3), cast.TimeFormat("s")), 3*time.Second)
	assert.Equal(t, cast.ToDuration(cast.Uint8Ptr(3), cast.TimeFormat("s")), 3*time.Second)
	assert.Equal(t, cast.ToDuration(cast.Uint16Ptr(3), cast.TimeFormat("s")), 3*time.Second)
	assert.Equal(t, cast.ToDuration(cast.Uint32Ptr(3), cast.TimeFormat("s")), 3*time.Second)
	assert.Equal(t, cast.ToDuration(cast.Uint64Ptr(3), cast.TimeFormat("s")), 3*time.Second)
